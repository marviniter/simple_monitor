...
class Features(object):
    ...
    def run(self,argv):
        ...
        #以token的方式进行身份验证（优先执行）
        self.token_auth()
        
        #以用户名密码的方式进行身份验证，验证通过self.__auth()返回值为True
        if  self.__auth():
    ...
    def __auth(self):
       ...
       while count < 3:
            ...
            try:
                ...
                sys.exit("\033[31;1mhaven't set Operational platform account yet ,please login http://localhost:8000/admin find 'Platform账户' and create an account first!\033[0m")
       else:
